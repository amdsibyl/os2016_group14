...
void display(void)
{
	...
	for(int i=0;i<NUM_BARBERS;i++){
		if(isBusy[i]){
			barBusy.blendTexRotate(50+150*i,screenHeight-150,1,1);
			cusPic[cutting[i]-1].blendTexRotate(50+150*i,screenHeight-150,0.95,0.95);
		}
		else
			barSleep.blendTexRotate(50+150*i,screenHeight-150,1,1);
	}
	/* barber 1 */
	/*if(isBusy[0]){
